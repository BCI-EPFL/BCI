...
% cross validation Baseline-MI termination
c=cvpartition( EpochTraining.BaseMIterm.labels,'Kfold',10);

for i=1:10
train=TrainingData.BaseMIterm(c.training(i),:);
test=TrainingData.BaseMIterm(c.test(i),:);
...
figure
imagesc([4 40],[1 16],PowerTraining.CrossBaseMIterm)
title('Fisher scores - baseline vs MI termination');