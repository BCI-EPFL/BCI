...
% since Marco recorded his data at a later stage, we only have 3 runs for
% him, therefore the lenght of the concatenation is different
...
for j=1:FilesLength
    ...
    for i=1:size(chanlocs16,2)
        ...
        if j==1
            ...
            % plotting only the channels of interested (C3, CP3, Cz, C4, CP4)
...
%% saving external matrices for the grand average
...
%% Only plotting the spectrograms related to the wanted channels
...
suptitle(sprintf('Spectrogram onset CAR - %s', name));
...
%% plotting the wanted channels for the grand average
...
suptitle('Grand average offset CAR');

%% topoplot alpha band INITIATION
...
for i=1:16
    ...
    [spect_for_one_channel,t, f]=Spectrogram_function(filt_epoch_baseline_lap, filt_epoch_MI_lap, i,filt_epoch_baseline_lap.fs, filt_epoch_baseline_lap.fs-32, Cyclic_freq2);
...
%% topoplot alpha band TERMINATION
...
for i=1:16
    ...
    [spect_for_one_channel,t, f]=Spectrogram_function(filt_epoch_baseline_lap, filt_epoch_MI_lap_termination, i,filt_epoch_baseline_lap.fs, filt_epoch_baseline_lap.fs-32, Cyclic_freq2);
...
suptitle(sprintf('Topoplot Laplacian %s - alpha band - offset', name));

%% topoplot beta band INITIATION
...
suptitle(sprintf('Topoplot Laplacian %s - beta band- onset', name));

%% topoplot beta band TERMINATION
...
suptitle(sprintf('Topoplot Laplacian %s - beta band- offset', name));
