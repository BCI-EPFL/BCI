...
for iFold=1:nFold
    ...
    xTrain=cat(1,Folds.MITerm.data{[1:(iFold-1),(iFold+1):10]});
    ...
    [indexPower, power_feat] = rankfeat(xTrain, yTrain{iFold},  'fisher');
    ...
    for iClassifier =1:numel(Classifier)
        for iFeature= featuresList
            ...
            featuresSelected = xTrain(:,indexPower(1:iFeature));
...
for j=1:numel(Classifier)
    for i=2:(iFold)
 XRoc=cat(2,XRoc,XforRoc{j,i});
 YRoc=cat(2,YRoc,YforRoc{j,i});   
    end
    XRocC{j}=XRoc;
    YRocC{j}=YRoc;
    XRoc=[];
