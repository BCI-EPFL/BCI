...
for i=1:10
   ...
   for j=1:numel(Classifier)
    for Nsel=1:20
       classifier.MITerm=fitcdiscr(TrainingFolds.MITerm.data(:,ind.MITerm(1:Nsel)),TrainingFolds.MITerm.labels,'discrimtype', Classifier{1,j});
       [yhat.MITerm,PosteriorProb.MITerm,~]=predict(classifier.MITerm,ValidationFold.MITerm.data(:,ind.MITerm(1:Nsel)));
...
cont=1;
k=0;
Folds.MITerm.data{j}= Folds.Base.data;
Folds.MITerm.labels{j}=Folds.Base.labels;
Folds.Base.data=[];
Folds.Base.labels=[];
end
